...
**Что сделано:**
- `services/draft.py` — добавлена функция `recommend_picks()` для рекомендации 3-5 героев на основе драфта
- Алгоритм ранжирования с 4 компонентами: контрпик вражеского состава (35%), синергия с союзниками (25%), мета-винрейт (25%), личный опыт (15%)
- `_Candidate` — внутренний dataclass для ранжирования кандидатов по score
- Параллельные запросы matchup-данных по вражеским героям через `asyncio.gather`
- Синергия с союзниками через отдельные matchup-запросы `with_heroes`
- Нормализация counter/synergy скоров в диапазон [0, 1]
- `_enrich_with_llm()` — обогащение рекомендаций пояснениями от LLM (с fallback на `_basic_reason()`)
- `_build_basic_recommendations()` — fallback без LLM для базовых пояснений
- `_basic_reason()` — генерация текстового пояснения из метрик кандидата
- Фильтрация уже выбранных героев (allies + enemies)
- Добавлена зависимость `cloudscraper` в `pyproject.toml` (требуется для `clients/stratz.py`)

**Тесты (tests/services/test_draft.py):**
- `TestCandidate` — 4 теста: расчёт score с/без личного винрейта, генерация basic_reason
- `TestRecommendPicks` — 5 тестов: возврат рекомендаций, исключение уже выбранных, обогащение LLM, пустой мета, наличие reason
- Все 19 тестов в файле прошли ✅

**Тест-шаги:**
- Шаг 1: `test_returns_recommendations` — вызов с тестовым драфтом возвращает 3-5 героев ✅
- Шаг 2: `test_each_recommendation_has_reason` — каждый герой имеет непустую причину ✅
- Шаг 3: `test_excludes_already_picked` — рекомендации не включают уже выбранных героев ✅
- `uv run ruff check .` — 0 ошибок ✅
- `uv run pytest tests/services/test_draft.py` — 19/19 тестов ✅

**Заметки для следующей итерации:**
- TASK-026 разблокирует: TASK-027 (хендлер /draft: приём скриншота + рекомендации)
- `recommend_picks()` принимает клиенты как аргументы (DI), совместимо с паттерном проекта
- `cloudscraper` теперь в зависимостях (нужен для Stratz клиента)
- Существующие падающие тесты (41 шт) — проблемы с stratz/cloudscraper моками, ContentType import в aiogram, format_build и др. — не связаны с TASK-026
- Для тестов draft не нужны реальные API — всё мокается через AsyncMock и patch
...
**Что сделано:**
- `prompts/draft_recognition.txt` — системный промпт для Vision-распознавания драфта. Содержит полный список 126 героев Dota 2 с hero_id. Формат вывода: `allies:id1,id2|enemies:id1,id2|role:N|confidence:X.X`. Обрабатывает edge-cases: неполный драфт, нечитаемый скриншот.
- `prompts/draft_recommendation.txt` — промпт для рекомендации 3-5 пиков. Учитывает контрпики, синергию, мету, личный опыт. Формат: `hero_id|имя_ru|причина` (по строке). Критерии ранжирования описаны.
- `prompts/match_summary.txt` — промпт для анализа матча с советами по роли. Разные метрики для каждой из 5 ролей. Сравнение с медианой. Структурированный формат ответа (до 2000 символов).
- `prompts/build_situational.txt` — промпт для ситуативной адаптации билда под вражеский состав. Правила адаптации по типам угроз (контроль, маг. урон, физ. урон, невидимость, хил). Лимит 1500 символов.

**Тест-шаги:**
- Шаг 1: Все 4 промпта загружаются через `load_prompt()` и содержат чёткие инструкции ✅
- Шаг 2: `draft_recognition.txt` содержит полный список героев (Anti-Mage, Largo, Kez, Ringmaster и т.д.) ✅
- Шаг 3: Формат вывода специфицирован для каждого промпта ✅
- `uv tool run ruff check .` — 0 ошибок ✅
- `uv tool run pytest tests/test_core.py` — 26/26 тестов ✅

**Заметки для следующей итерации:**
- Все промпты совместимы с парсерами в `clients/llm.py` (`_parse_draft_recognition`, `_parse_pick_recommendations`)
- TASK-024 разблокирует: TASK-025 (сервис распознавания драфта), TASK-026 (рекомендация пиков), TASK-028 (LLM-совет по матчу), TASK-029 (ситуативная адаптация билдов)
- Промпты на русском с примерами, структурированы, содержат лимиты длины ответа
...
## 2026-03-07 — TASK-035: Rate limiting на уровне бота (DONE)

**Что сделано:**
- `bot/middlewares/throttle.py` — полностью переработан ThrottleMiddleware:
  - Выделен класс `_RateBucket` для управления отдельными корзинами лимитов
  - Раздельные лимиты: текстовые сообщения (30/мин) и изображения (5/мин)
  - Автоматическое определение типа контента через `ContentType.PHOTO`
  - Информативные сообщения при блокировке (разные для текста и изображений)
  - Обратная совместимость: `rate_limit`/`window_sec` по-прежнему работают
- Глобальная функция `check_llm_limit(telegram_id)` — отдельный лимит для LLM-запросов (10/час)
  - Возвращает `True` если запрос разрешён, `False` — если лимит исчерпан
  - Предназначена для вызова из хендлеров premium-фич
- `tests/test_middlewares.py` — добавлено 7 новых тестов (было 25, стало 32):
  - Раздельный лимит для изображений
  - Независимость лимитов текста и изображений
  - Текст предупреждения для изображений
  - LLM лимит: разрешение, блокировка, per-user, сброс после окна

**Тест-шаги:**
- 32 текстовых/фото сообщения: текстовый лимит 30/мин работает ✅
- 6 изображений при лимите 5/мин: блокировка ✅
- LLM лимит 10/час: блокировка после 10, сброс после окна ✅
- `uv tool run ruff check .` — 0 ошибок ✅
- `uv tool run pytest tests/test_middlewares.py` — 32/32 тестов ✅

**Заметки для следующей итерации:**
- `check_llm_limit()` нужно интегрировать в хендлеры premium-фич (TASK-027, 028, 029)
- Остальные тесты (14 файлов) — предсуществующая проблема коллекции (httpx, cloudscraper)
- Приоритетные pending задачи:
  - TASK-024 (LLM-промпты, medium) — блокирует TASK-025, 028, 029
  - TASK-030 (scheduler, medium)
  - TASK-032 (подписки, medium)
